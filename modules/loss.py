...
def skeletal_losses(prediction, target, dataset, mask):
    ...
    adj_loss = bce_logits_fn(
        adj_hat.data[mask.byte()],
        target.data[:, (dataset.continuous_feature_dim + dataset.discrete_feature_dim):][mask.byte()],
    )
